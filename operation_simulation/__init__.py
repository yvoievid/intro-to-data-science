...
for spec in env_specs:
    register(
        id='operation_' + spec.id,
        entry_point='operation_simulation.envs.openai:MultiAgentWrapper',
...
for i, observability in enumerate([False, True]):
    ...
    for clock in [False, True]:
        register(
            ...
            entry_point='operation_simulation.envs.checkers:Checkers',
            kwargs={'full_observable': observability, 'step_cost': -0.01, 'clock': clock}
        )
        register(
            id='Switch2-v{}'.format(i + (2 if clock else 0)),
            entry_point='operation_simulation.envs.switch:Switch',
    ...
    for num_max_cars in [4, 10]:
        register(
            ...
            entry_point='operation_simulation.envs.traffic_junction:TrafficJunction',
            kwargs={'full_observable': observability, 'n_max': num_max_cars}
        )

    register(
        id='Lumberjacks-v' + str(i),
        entry_point='operation_simulation.envs.lumberjacks:Lumberjacks',
...
register(
    id='PongDuel-v0',
    entry_point='operation_simulation.envs.pong_duel:PongDuel',
)
...
for game_info in [[(5, 5), 2, 1], [(7, 7), 4, 2]]:  # [(grid_shape, predator_n, prey_n),..]
    ...
    register(
        ...
        entry_point='operation_simulation.envs.predator_prey:PredatorPrey',
        kwargs={
