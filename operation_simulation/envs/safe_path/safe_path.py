...
class SafePath(gym.Env):
    ...
    def _get_info(self):
        return {
            "distance": np.linalg.norm(
                self.prey_pos[0] - self._target_location, ord=1
            )
        }
        ...
        for prey_i in range(self.n_preys):
            if self._prey_alive[prey_i]:
                ...
                if predator_neighbour_count >= 1:
                    ...
                    _reward += additional_reward
                    for agent_i in range(self.n_agents):
                        rewards[agent_i] += _reward
                       
                prey_move = None
                if self._prey_alive[prey_i] or np.array_equal(self.prey_pos[prey_i], self._target_location):
    ...
    def render(self, mode='human'):
        ...
        fill_cell(img, self._target_location, cell_size=CELL_SIZE, fill=PREY_COLOR)
